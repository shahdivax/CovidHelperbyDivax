...
class covidhelp:
    ...
    def icon(self):
        photo = PhotoImage(
            file="C:/Users/lenovo/Desktop/covid-help/logos/LogoMakr-0WzAEA.gif"
        )
    ...
    def receive(self):
        labelframeR = LabelFrame(self.root,
                                 text="Get Help",
                                 bg="sky blue",
                                 font="helvetica 15")
        ...
        Button(
            labelframeR,
            text="Search",
            command=self.view_donars,
            bg="blue",
            fg="black",
            font="helvetica 12",
        ).grid(row=11, column=1, sticky=E, padx=5, pady=5)
        Button(
            labelframeR,
            text="Delete False",
            command=self.on_delet_selected_button_clicked,
            bg="red",
            fg="white",
            font="helvetica 12",
        ).grid(row=11, column=2, sticky=E, padx=5, pady=5)

    def create_scrollbar(self):
        self.scrollbar = Scrollbar(orient="vertical", command=self.tree.yview)
        self.scrollbar.grid(row=13,
                            column=5,
                            columnspan=3,
                            rowspan=10,
                            sticky="sn")
    ...
    def add_donation(self):
        if self.validated():
            if self.number():
                ...
                self.message["text"] = "New Donar {} added".format(
                    self.namefield.get())
    ...
    def view_donars(self):
        ...
        if self.Rvalidated() and self.Tvalidated():
            ...
            donar_entries = self.execute_db_query(
                query,
                (
                    name,
                    thing,
    ...
    def validated(self):
        return (len(self.namefield.get()) != 0
                ...
                and len(self.cityfield.get()) != 0
                and len(self.thingfield.get()) != 0)
